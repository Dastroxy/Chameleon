...
export default function GameRoom({ roomId, user, sessionId, playerName, onLeave }: Props) {
  ...
  if (!room) {
    return (
      <div className="min-h-screen checker-bg flex items-center justify-center">
        <div className="text-primary animate-pulse font-bold text-lg">Loading room...</div>
      </div>
    )
  }
  ...
  async function startGame() {
    if (players.length < 3) { alert('Need at least 3 players to start!'); return }
    ...
    if (!secretWord) {
      alert('Something went wrong generating the game. Try again.')
      return
    }
  ...
  if (room.phase === 'voting' || room.phase === 'chameleon_guess') {
    return (
      <>
        <VotingPhase room={room} user={user} sessionId={sessionId} roomRef={roomRef} onLeave={leaveRoom} />
        {showChat && <Chat room={room} user={user} sessionId={sessionId} roomRef={roomRef} onClose={() => setShowChat(false)} />}
      </>
    )
  }
  ...
  if (room.phase === 'results') {
    return <Results room={room} user={user} sessionId={sessionId} roomRef={roomRef} onLeave={leaveRoom} />
  }
  ...
  return (
    <div className="min-h-screen checker-bg font-display text-gray-900 flex flex-col">
      <div className="flex items-center bg-white p-4 border-b border-gray-200 justify-between sticky top-0 z-10 shadow-sm">
        <button onClick={leaveRoom} className="size-10 flex items-center justify-center rounded-full hover:bg-gray-100">
          <span className="material-symbols-outlined text-gray-500">arrow_back</span>
        </button>
        <div className="text-center">
          <h2 className="text-lg font-black">Game Lobby</h2>
      ...
      <div className="flex-1 overflow-y-auto pb-32 px-4 py-4 space-y-4">
        <div className="checker-hero rounded-2xl p-6 flex flex-col items-center text-center shadow-md">
          ...
          <p className="text-green-100 text-sm mt-1">Waiting for players to join...</p>
        </div>
        ...
        <div className="flex gap-3">
          <div className="flex-1 rounded-2xl border border-gray-100 bg-white p-4 text-center shadow-sm">
            ...
            <p className="text-gray-400 text-xs uppercase tracking-wider mt-1">Players</p>
          </div>
          <div className="flex-1 rounded-2xl border border-gray-100 bg-white p-4 text-center shadow-sm">
            <p className="text-secondary text-3xl font-black">{8 - players.length}</p>
            <p className="text-gray-400 text-xs uppercase tracking-wider mt-1">Empty Slots</p>
          </div>
        </div>

        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
          <p className="text-xs text-gray-400 uppercase tracking-wider mb-2 font-bold">Share Room Code</p>
          <div className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3 border-2 border-dashed border-primary/30">
            <code className="text-primary font-black text-xl tracking-widest mr-2">{roomId}</code>
            <button onClick={() => { navigator.clipboard.writeText(roomId); alert('Copied!') }}
              className="text-gray-400 hover:text-primary flex-shrink-0 transition-colors">
              ...
              <div key={p.uid} className="flex items-center justify-between p-3 rounded-xl bg-white border border-gray-100 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-primary uppercase tracking-widest font-bold">{p.isHost ? '👑 Host' : 'Player'}</p>
                  </div>
                </div>
        ...
        {isHost ? (
          <>
            <button onClick={startGame} disabled={players.length < 3}
              className="w-full flex items-center justify-center gap-3 bg-primary text-white h-14 rounded-xl font-black text-lg shadow-md active:scale-95 transition-all disabled:opacity-40">
          ...
          <div className="w-full py-3 text-center text-gray-400 text-sm font-medium">
